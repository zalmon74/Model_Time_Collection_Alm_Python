...
def imitator_bink(mat_mi_n: matrix, arr_mi_epi: array):
  ...
  ci = generator_ci_for_all_sat(mat_av_sig_for_sat, vec_count_psk
                               ,vec_l_psk, arr_n_as, mat_sync, mat_pps
                               ,mat_kps, mat_prior)
  return ci
...
def decode_arr_mi_epi(arr_mi_epi: array):
  ...
  for ind_str in range(count_str):
    ...
    for ind_sig in range(count_sig):
      ...
      if (arr_mi_epi[ind_sat, ind_str, ind_sig] == None):
        mat_t_oi[ind_str, ind_sig ] = gc.NOT_BEL_STR
        mat_kps[ind_str, ind_sig  ] = gc.NOT_BEL_STR
        mat_sync[ind_str, ind_sig ] = gc.NOT_BEL_STR
        mat_pps[ind_str, ind_sig  ] = gc.NOT_BEL_STR
        mat_prior[ind_str, ind_sig] = gc.NOT_BEL_STR
        mat_pvs[ind_str, ind_sig  ] = gc.NOT_BEL_STR
        continue
      else:
...
def decode_mat_av_sig_for_sat(mat_mi_n: matrix):
  ...
  for ind_sat in range(count_sat):
    ...
    for ind_sig in range(count_sig):
      temp_value = 0 
      if (mat_mi_n[ind_sat, ind_sig] != None):
        temp_value = 1
...
def generator_ci_for_all_sat(mat_av_sig_for_sat: matrix, vec_count_psk: array
                            ,vec_l_psk: array, arr_n_as: array
                            ,mat_sync: matrix, mat_pps: matrix, mat_kps: matrix
                            ,mat_prior: matrix):
  """
...
def generator_ci_for_all_signals(vec_av_sig_for_sat: array, vec_count_psk: array
                                ,vec_l_psk: array, mat_n_as: matrix
                                ,mat_sync: matrix, mat_pps: matrix
                                ,mat_kps: matrix, mat_prior: matrix
                                ,count_sat: int):
  """
...
def generator_ci_for_signal(count_psk: int, size_psk: int, vec_sync: array
                           ,vec_pps: array, vec_kps: array, vec_prior: array
                           ,vec_n_as: array, time_slot: int, count_sat: int):
  """
  ...
  dic_num_psk_in_str = formation_dic_num_psk_in_ind_str(count_psk, vec_sync
                                                       ,vec_pps, vec_kps
                                                       ,vec_prior)
...
def formation_dic_num_psk_in_ind_str(count_psk: int, vec_sync: array
                                    ,vec_pps: array, vec_kps: array
                                    ,vec_prior: array):
  """
  ...
  for num_psk in range(count_psk):
    ...
    vec_str_cur_psk = full(vec_ind_str_cur_psk.size, gc.NOT_BEL_STR, dtype = int)
    ...
    vec_loc_ind_tr_str = nonzero(vec_str_cur_psk != gc.NOT_BEL_STR)[0]
